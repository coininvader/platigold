...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePlatiGoldAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePlatiGoldAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePlatiGoldAmount = strAnonymizePlatiGoldAmount.remove(strAnonymizePlatiGoldAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePlatiGoldAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePlatiGoldAmount*COIN) nMaxToAnonymize = nAnonymizePlatiGoldAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePlatiGoldAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePlatiGoldAmount));
        strAnonymizePlatiGoldAmount = strAnonymizePlatiGoldAmount.remove(strAnonymizePlatiGoldAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePlatiGoldAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizePlatiGoldAmount == 0){
