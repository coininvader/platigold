...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("PlatiGold"))
        return false;
...
{
    // Convert PlatiGold:// to PlatiGold:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("PlatiGold://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "PlatiGold:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PlatiGold.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for PlatiGold.lnk
...
{
    return GetAutostartDir() / "PlatiGold.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a PlatiGold.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=PlatiGold\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the PlatiGold app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add PlatiGold app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PlatiGold-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PlatiGold-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("PlatiGold-Qt"));
