...
void Shutdown()
{
    ...
    RenameThread("PlatiGold-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: PlatiGold.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: PlatiGoldd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePlatiGoldamount=<n> " + _("Keep N PlatiGold anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. PlatiGold is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PlatiGold is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("PlatiGold version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "PlatiGold server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of PlatiGold") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart PlatiGold to complete") << "\n";
    ...
    nAnonymizePlatiGoldAmount = GetArg("-anonymizePlatiGoldamount", 0);
    if(nAnonymizePlatiGoldAmount > 999999) nAnonymizePlatiGoldAmount = 999999;
    if(nAnonymizePlatiGoldAmount < 2) nAnonymizePlatiGoldAmount = 2;
    ...
    LogPrintf("Anonymize PlatiGold Amount %d\n", nAnonymizePlatiGoldAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1PTG+1000 == (.1PTG+100)*10
       10PTG+10000 == (1PTG+1000)*10
    */
