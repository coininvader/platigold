...
int nAnonymizePlatiGoldAmount = 1000;
...
#else
    const char* pszModule = "PlatiGold";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PlatiGold
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PlatiGold
    // Mac: ~/Library/Application Support/PlatiGold
    // Unix: ~/.PlatiGold
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PlatiGold";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "PlatiGold.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "PlatiGoldd.pid"));
